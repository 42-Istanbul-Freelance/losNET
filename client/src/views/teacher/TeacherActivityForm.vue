<template>
  <div class="activity-form-page">
    <div class="page-header">
      ...
      <p class="page-subtitle">{{ isEdit ? 'Etkinlik bilgilerini güncelle' : 'Okulunuzda yapılacak bir etkinlik oluşturun ve öğrencileri davet edin.' }}</p>
    </div>
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input v-model="form.location" type="text" class="form-input" placeholder="Örn: Okul, Kütüphane, Park" />
          </div>
          <div class="form-group">
        ...
        <div class="form-actions">
          <router-link :to="backPath" class="btn btn-outline">İptal</router-link>
...
export default {
  ...
  setup() {
    ...
    const backPath = computed(() => authStore.role === 'admin' ? '/admin/dashboard' : '/teacher/dashboard')
    ...
    onMounted(async () => {
      ...
      try {
        ...
        availableStudents.value = res.data.activities
          ?.flatMap(a => a.participantStudents || [])
          .map(p => p.student)
          .filter((s, i, arr) => s && arr.findIndex(item => item._id === s._id) === i)
